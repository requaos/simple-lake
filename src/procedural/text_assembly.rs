...
pub fn assemble_description(
    ...
    let opening = fragments.openings.choose(rng)
        .expect("No opening fragments")
        .clone();
    let conflict = fragments.conflicts.choose(rng)
        .expect("No conflict fragments")
        .clone();
    let stakes = fragments.stakes.choose(rng)
...
pub fn assemble_choice_text(
    text_fragments: &[String],
    rng: &mut impl Rng,
) -> String {
    text_fragments.choose(rng)
...
fn substitute_variables(
    ...
    if text.contains("{relationship_type}") {
        let relationship = variables.relationship_types.choose(rng)
